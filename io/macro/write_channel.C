...
namespace
{
    ...
    std::vector<double> parse_bins_csv(const char *text, int expected_nbins)
    {
        if (!text || !*text)
                   ...
                   const char *background_cache_key = nullptr)
{
    macro_utils::run_macro("write_channel", [&]() {
        ...
        channel.spec.selection_expr = selection_expr;
        channel.spec.nbins = sig.spec.nbins;
        channel.spec.xmin = sig.spec.xmin;
        channel.spec.xmax = sig.spec.xmax;
        channel.data = parse_bins_csv(data_bins_csv, sig.spec.nbins);
